...
def plot_hourly_activity(df: pd.DataFrame) -> plt.Figure:
    """Histogram aktywnosci AIS per godzina doby."""
    fig, ax = plt.subplots(figsize=(12, 5))
    df = df.copy()
    df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
    df = df.dropna(subset=["timestamp"])
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f"Srednia: {mean_hour:.1f}h",
    )
    ax.set_title(
        "Aktywnosc AIS per godzina doby - rosyjska flota cieni",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xlabel("Godzina (UTC)")
    ax.set_ylabel("Liczba pingow AIS")
...
def plot_anomaly_map(
    ...
    """Mapa tras podejrzanych statkow z anomaliami i podkladem geograficznym."""
    ...
    for mmsi in top_mmsi:
        ...
        # odfiltruj nierealne współrzędne (błędne dane AIS, np. stacje bazowe)
        track = track[
            (track["latitude"].between(-85, 85))
            & (track["longitude"].between(-180, 180))
        ]
        if track.empty:
            continue
    ...
    anom_subset = anom_subset[
        (anom_subset["latitude"].between(-85, 85))
        & (anom_subset["longitude"].between(-180, 180))
    ]
    for mmsi in top_mmsi:
        color = color_map[mmsi]
    ...
    if all_x and all_y:
        ...
        # fallback: brak poprawnych danych pozycyjnych
        ax.text(
            0.5,
            0.5,
            "Brak poprawnych danych pozycyjnych",
            ha="center",
            va="center",
            transform=ax.transAxes,
        )
    ...
    ax.set_title(
        ...
        "Linie = pelna trasa AIS  |  duza kropka = wykryta anomalia",
        fontsize=13,
        fontweight="bold",
        pad=12,
    )
